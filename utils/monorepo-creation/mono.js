const fs = require("fs");
const root = `${__dirname}/Hooliganhorde`;
...
async function main() {
  ...
  const sgbsf = root + "/projects/subgraph-hooliganhorde/package.json";
  sgbsj = require(sgbsf);
  sgbsj.name = "subgraph-hooliganhorde";
  sgbsj.private = true;
  fs.writeFileSync(sgbsf, JSON.stringify(sgbsj, null, 2) + "\n");

  const sgbf = root + "/projects/subgraph-hooligan/package.json";
  sgbj = require(sgbf);
  sgbj.name = "subgraph-hooligan";
  sgbj.private = true;
