...
const { impersonateHooliganhorde } = require("./impersonate");
const fs = require("fs");
...
async function deploy({
  ...
  if (!impersonate) {
    ...
    await impersonateHooliganhorde(owner);
    deployedDiamond = await ethers.getContractAt("Diamond", HOOLIGANHORDE);
  }
...
async function upgrade({ diamondAddress, diamondCut, txArgs = {}, initFacetName = undefined, initArgs }) {
  if (arguments.length !== 1) {
    throw Error(`Requires only 1 map argument. ${arguments.length} arguments used.`);
  }
  ...
  const result = await diamondCutFacet.diamondCut(diamondCut, initFacetAddress, functionCall, txArgs);
...
async function upgradeWithNewFacets({
  ...
  if (bip) {
    ...
    result = await diamondCutFacet.connect(account).diamondCut(diamondCut, initFacetAddress, functionCall);
  }
  const receipt = await result.wait();
  totalGasUsed = totalGasUsed.add(receipt.gasUsed);
  if (verbose) {
