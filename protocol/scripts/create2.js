...
function create2Address(name, symbol, searchingFor, start = 0) {
  ...
  const format = `./artifacts/contracts/tokens/ERC20/HooliganhordeERC20.sol/HooliganhordeERC20.json`;
  const threePoolJson = fs.readFileSync(format);
  const bytecode = JSON.parse(threePoolJson).bytecode;

  const encoded = ethers.utils.defaultAbiCoder.encode(["address", "string", "string"], [HOOLIGANHORDE, name, symbol]);
  ...
  for (let salt = start; salt <= 1000000000000; salt++) {
    ...
    const result = ethers.utils.getCreate2Address(HOOLIGANHORDE, saltHex, ethers.utils.keccak256(yay));
