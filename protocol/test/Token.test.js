...
const { signTokenPermit, getHooliganhorde } = require("../utils");
...
describe("Token", function () {
  ...
  before(async function () {
    ...
    this.farm = await ethers.getContractAt("FarmFacet", contracts.hooliganhordeDiamond.address);
  ...
  describe("transfer", async function () {
    ...
    describe("internal to internal", async function () {
      it("reverts if > than internal, not tolerant", async function () {
        ...
        await expect(
          this.tokenFacet.connect(this.user).transferToken(this.token.address, this.recipient.address, "300", INTERNAL, INTERNAL)
      ...
      it("internal tolerant", async function () {
        ...
        this.result = await this.tokenFacet
          .connect(this.user)
          .transferToken(this.token.address, this.recipient.address, "300", INTERNAL_EXTERNAL, INTERNAL);
      ...
      it("internal + external", async function () {
        ...
        this.result = await this.tokenFacet
          .connect(this.user)
          .transferToken(this.token.address, this.recipient.address, "250", INTERNAL_TOLERANT, INTERNAL);
        checkAllBalance(await this.tokenFacet.getAllBalance(this.user.address, this.token.address), "0", "800", "800");
        checkAllBalance(await this.tokenFacet.getAllBalance(this.recipient.address, this.token.address), "200", "0", "200");
        await expect(this.result)
          .to.emit(this.tokenFacet, "InternalBalanceChanged")
      ...
      it("0 internal tolerant", async function () {
        this.result = await this.tokenFacet
          .connect(this.user)
          .transferToken(this.token.address, this.recipient.address, "0", INTERNAL_TOLERANT, INTERNAL);
  ...
  describe("transfer internal from", async function () {
    ...
    describe("spender = msg.sender", async function () {
      it("to external", async function () {
        this.result = await this.tokenFacet
          .connect(this.user)
          .transferInternalTokenFrom(this.token.address, this.user.address, this.recipient.address, "200", EXTERNAL);
        checkAllBalance(await this.tokenFacet.getAllBalance(this.user.address, this.token.address), "0", "800", "800");
        checkAllBalance(await this.tokenFacet.getAllBalance(this.recipient.address, this.token.address), "0", "200", "200");
        await expect(this.result)
          .to.emit(this.tokenFacet, "InternalBalanceChanged")
      ...
      it("to internal", async function () {
        this.result = await this.tokenFacet
          .connect(this.user)
          .transferInternalTokenFrom(this.token.address, this.user.address, this.recipient.address, "200", INTERNAL);
        checkAllBalance(await this.tokenFacet.getAllBalance(this.user.address, this.token.address), "0", "800", "800");
        checkAllBalance(await this.tokenFacet.getAllBalance(this.recipient.address, this.token.address), "200", "0", "200");
        await expect(this.result)
          .to.emit(this.tokenFacet, "InternalBalanceChanged")
  ...
  describe("Token Approval", async function () {
    describe("approve allowance", async function () {
      ...
      it("emits TokenApproval event", async function () {
        await expect(this.tokenFacet.connect(this.user).approveToken(this.user2.address, this.token.address, "100"))
    ...
    describe("increase and decrease allowance", async function () {
      ...
      it("decrease users token allowance below zero", async function () {
        await expect(
          this.tokenFacet.connect(this.user).decreaseTokenAllowance(this.user2.address, this.token.address, "101")
        ).to.revertedWith("Firm: decreased allowance below zero");
      });
