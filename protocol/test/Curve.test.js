...
const { HOOLIGAN, THREE_CURVE, THREE_POOL, HOOLIGAN_3_CURVE, STABLE_FACTORY, USDC } = require("./utils/constants");
...
describe("Curve", function () {
  before(async function () {
    ...
    await this.hooligan.connect(user).approve(this.diamond.address, "100000000000");
    await this.hooligan.connect(user2).approve(this.diamond.address, "100000000000");
    ...
    await this.hooliganMetapool.connect(user).approve(this.diamond.address, to18("100000000000"));
  ...
  describe("Add Liquidity", async function () {
    describe("To external", async function () {
      ...
      it("updates hooliganhorde balances", async function () {
        expect(await this.hooligan.balanceOf(this.token.address)).to.be.eq("0");
        expect(await this.threeCurve.balanceOf(this.token.address)).to.be.eq("0");
        expect(await this.hooliganMetapool.balanceOf(this.token.address)).to.be.eq("0");
      });

      it("adds lp tokens to external balance", async function () {
        expect(await this.token.getExternalBalance(userAddress, this.hooliganMetapool.address)).to.be.equal(to18("2000"));
      });

      it("adds lp tokens to internal balance", async function () {
        expect(await this.token.getInternalBalance(userAddress, this.hooliganMetapool.address)).to.be.equal(to18("0"));
      });
    });
    ...
    describe("To internal", async function () {
      beforeEach(async function () {
        ...
        this.result = await this.curve
          .connect(user)
          .addLiquidity(HOOLIGAN_3_CURVE, STABLE_FACTORY, [to6("500"), to18("500")], to18("1000"), EXTERNAL, INTERNAL);
      });
      ...
      it("updates hooliganhorde balances", async function () {
        expect(await this.hooligan.balanceOf(this.token.address)).to.be.eq("0");
        expect(await this.threeCurve.balanceOf(this.token.address)).to.be.eq("0");
        expect(await this.hooliganMetapool.balanceOf(this.token.address)).to.be.eq(to18("1000"));
      });
      ...
      it("adds lp tokens to internal balance", async function () {
        expect(await this.token.getInternalBalance(userAddress, this.hooliganMetapool.address)).to.be.equal(to18("1000"));
      });
    });
  });
  ...
  describe("Remove Liqudity", async function () {
    describe("To external", async function () {
      beforeEach(async function () {
        await this.curve
          .connect(user)
          .removeLiquidity(this.hooliganMetapool.address, STABLE_FACTORY, to18("500"), [to6("250"), to18("250")], EXTERNAL, EXTERNAL);
      });
      ...
      it("adds lp tokens to internal balance", async function () {
        ...
        expect(await this.token.getInternalBalance(userAddress, this.hooligan.address)).to.be.equal("0");
      });

      it("adds lp tokens to external balance", async function () {
        expect(await this.token.getExternalBalance(userAddress, this.threeCurve.address)).to.be.equal(to18("250"));
        expect(await this.token.getExternalBalance(userAddress, this.hooligan.address)).to.be.equal(to6("9250"));
      });
    });
    ...
    describe("To internal", async function () {
      beforeEach(async function () {
        await this.curve
          .connect(user)
          .removeLiquidity(this.hooliganMetapool.address, STABLE_FACTORY, to18("500"), [to6("250"), to18("250")], EXTERNAL, INTERNAL);
      });
      ...
      it("updates hooliganhorde balances", async function () {
        expect(await this.hooligan.balanceOf(this.token.address)).to.be.eq(to6("250"));
      ...
      it("adds lp tokens to internal balance", async function () {
        ...
        expect(await this.token.getInternalBalance(userAddress, this.hooligan.address)).to.be.equal(to6("250"));
      });

      it("adds lp tokens to external balance", async function () {
        expect(await this.token.getExternalBalance(userAddress, this.threeCurve.address)).to.be.equal(to18("0"));
        expect(await this.token.getExternalBalance(userAddress, this.hooligan.address)).to.be.equal(to6("9000"));
      });
    });
  });
  ...
  describe("Remove Liqudity Inbalance", async function () {
    describe("To external", async function () {
      ...
      it("adds lp tokens to external balance", async function () {
        ...
        expect(await this.token.getExternalBalance(userAddress, this.hooligan.address)).to.be.equal(to6("9400"));
      });
    });
    ...
    describe("To internal", async function () {
      ...
      it("updates hooliganhorde balances", async function () {
        expect(await this.hooligan.balanceOf(this.token.address)).to.be.eq(to6("400"));
      ...
      it("adds lp tokens to internal balance", async function () {
        ...
        expect(await this.token.getInternalBalance(userAddress, this.hooligan.address)).to.be.equal(to6("400"));
      });
  ...
  describe("Remove Liqudity one token", async function () {
    describe("To external", async function () {
      ...
      it("adds lp tokens to external balance", async function () {
        ...
        expect(await this.token.getExternalBalance(userAddress, this.hooligan.address)).to.be.equal("9491960239");
      });
    });
    ...
    describe("To internal", async function () {
      ...
      it("updates hooliganhorde balances", async function () {
        expect(await this.hooligan.balanceOf(this.token.address)).to.be.eq("491960239");
        expect(await this.threeCurve.balanceOf(this.token.address)).to.be.eq("0");
        expect(await this.hooliganMetapool.balanceOf(this.token.address)).to.be.eq("0");
      });
      ...
      it("adds lp tokens to internal balance", async function () {
        expect(await this.token.getInternalBalance(userAddress, this.threeCurve.address)).to.be.equal("0");
        expect(await this.token.getInternalBalance(userAddress, this.hooligan.address)).to.be.equal("491960239");
      });
  ...
  describe("farm LP and Deposit", async function () {
    beforeEach("add LP and Deposits", async function () {
      ...
      const deposit = await this.firm.interface.encodeFunctionData("deposit", [this.hooliganMetapool.address, to18("1000"), INTERNAL]);

      await this.threeCurve.mint(user2Address, to18("500"));
      await this.hooligan.mint(user2Address, to6("500"));
      await this.threeCurve.connect(user2).approve(this.firm.address, to18("5000000"));
      await this.hooligan.connect(user2).approve(this.firm.address, to6("50000000"));

      await this.farm.connect(user2).farm([addLiquidity, deposit]);
    });

    it("add lp and deposit", async function () {
      const gameday = await this.gameday.gameday();
      const dep = await this.firm.getDeposit(user2Address, this.hooliganMetapool.address, gameday);
