...
describe("Ownership", function () {
  ...
  describe("ownership", async function () {
    describe("transfer", async function () {
      it("reverts if not owner", async function () {
        await expect(this.ownership.connect(user2).transferOwnership(user2Address)).to.be.revertedWith(
  ...
  describe("whitelist", async function () {
    it("reverts if not owner", async function () {
      await expect(
        this.whitelist
          .connect(user2)
          .whitelistToken(this.firmToken.address, this.firm.interface.getSighash("mockBDV(uint256 amount)"), "10000", "1")
    ...
    it("whitelists token", async function () {
      this.result = this.whitelist
        .connect(owner)
        .whitelistToken(this.firmToken.address, this.firm.interface.getSighash("mockBDV(uint256 amount)"), "10000", "1");
      const settings = await this.firm.tokenSettings(this.firmToken.address);
      expect(settings[0]).to.equal(this.firm.interface.getSighash("mockBDV(uint256 amount)"));
      expect(settings[1]).to.equal(1);
      expect(settings[2]).to.equal(10000);
      await expect(this.result)
        .to.emit(this.whitelist, "WhitelistToken")
        .withArgs(this.firmToken.address, this.firm.interface.getSighash("mockBDV(uint256 amount)"), 10000, 1);
    });
  });

  describe("dewhitelist", async function () {
    it("reverts if not owner", async function () {
      await expect(this.whitelist.connect(user2).dewhitelistToken(this.firmToken.address)).to.be.revertedWith(
    ...
    it("dewhitelists token", async function () {
      await this.whitelist
        .connect(owner)
        .whitelistToken(this.firmToken.address, this.firm.interface.getSighash("mockBDV(uint256 amount)"), "10000", "1");
      this.result = await this.whitelist.connect(owner).dewhitelistToken(this.firmToken.address);
      const settings = await this.firm.tokenSettings(this.firmToken.address);
      expect(settings[0]).to.equal("0x00000000");
      expect(settings[1]).to.equal(0);
      expect(settings[2]).to.equal(0);
      await expect(this.result).to.emit(this.whitelist, "DewhitelistToken").withArgs(this.firmToken.address);
    });
  });
  ...
  describe("pause", async function () {
    ...
    it("pauses", async function () {
      this.result = await this.pause.connect(owner).pause();
      expect(await this.gameday.paused()).to.equal(true);
  ...
  describe("unpause", async function () {
    ...
    it("unpauses", async function () {
      ...
      expect(await this.gameday.paused()).to.equal(false);
