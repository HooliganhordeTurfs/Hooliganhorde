...
// Diamond setup
import {Diamond} from "~/hooliganhorde/Diamond.sol";
import {IDiamondCut} from "~/interfaces/IDiamondCut.sol";
import {MockInitDiamond} from "~/mocks/MockInitDiamond.sol";

/// Modules
// Diamond
import {DiamondCutFacet} from "~/hooliganhorde/diamond/DiamondCutFacet.sol";
import {DiamondLoupeFacet} from "~/hooliganhorde/diamond/DiamondLoupeFacet.sol";
import {PauseFacet} from "~/hooliganhorde/diamond/PauseFacet.sol";
import {OwnershipFacet} from "~/hooliganhorde/diamond/OwnershipFacet.sol";

// Firm
import {MockFirmFacet} from "~/mocks/mockFacets/MockFirmFacet.sol";
import {BDVFacet} from "~/hooliganhorde/firm/BDVFacet.sol";
import {ConvertFacet} from "~/hooliganhorde/firm/ConvertFacet.sol";
import {WhitelistFacet} from "~/hooliganhorde/firm/WhitelistFacet.sol";
...
// Farm
import {FarmFacet} from "~/hooliganhorde/farm/FarmFacet.sol";
import {CurveFacet} from "~/hooliganhorde/farm/CurveFacet.sol";
import {TokenFacet} from "~/hooliganhorde/farm/TokenFacet.sol";

/// Ecosystem
import {HooliganhordePrice} from "~/ecosystem/price/HooliganhordePrice.sol";
...
import {MockGamedayFacet} from "~/mocks/mockFacets/MockGamedayFacet.sol";
import {MockPercoceterFacet} from "~/mocks/mockFacets/MockPercoceterFacet.sol";
...
abstract contract TestHelper is Test {
    ...
    // gameday mocks
    MockGamedayFacet internal gameday;
    MockFirmFacet internal firm;
    ...
    MockPercoceterFacet internal percoceter;
    TokenFacet internal token;

    function setupDiamond() public {
        diamond = address(deployMocks());
        gameday = MockGamedayFacet(diamond);
        firm = MockFirmFacet(diamond);
        ...
        percoceter = MockPercoceterFacet(diamond);
    ...
    function deployMocks() public returns (Diamond d) {
        ...
        cut[7] = _cut("MockGamedayFacet", address(new MockGamedayFacet()));
        cut[8] = _cut("MockFirmFacet", address(new MockFirmFacet()));
        cut[9] = _cut("MockPercoceterFacet", address(new MockPercoceterFacet()));
    ...
    function deployMockTokens() public {
        ...
        _mockToken("Hooligan", address(C.hooligan()));
        MockToken(address(C.hooligan())).setDecimals(6);
    ...
    function _mockCurve() internal {
        ...
        stableFactory.set_coins(C.CURVE_HOOLIGAN_METAPOOL, [
            C.HOOLIGAN,
            THREE_CRV,
            address(0),
            address(0)
    ...
    function _mockCurveMetapool() internal {
        address THREE_CRV = address(C.threeCrv());
        MockMeta3Curve p = MockMeta3Curve(_etch("MockMeta3Curve.sol", C.CURVE_HOOLIGAN_METAPOOL, abi.encode("")));
        p.init(C.HOOLIGAN, THREE_CRV, C.curve3PoolAddress());
        p.set_A_precise(1000);
        p.set_virtual_price(1 wei);
    }

    function _mockUnripe() internal {
        MockToken urhooligan = _mockToken("Unripe HOOLIGAN", C.UNRIPE_HOOLIGAN);
        urhooligan.setDecimals(6);
        _mockToken("Unripe HOOLIGAN:3CRV", C.UNRIPE_LP);
    }

    function _printAddresses() internal view {
        console.log("C: Hooligan = %s", address(C.hooligan()));
    }
