  ...
  while (degreeIndex <= 3) {
    ...
    if (f.signs[pieceIndex * 4 + degreeIndex] == 1) {
      positiveSum = math.add(
        positiveSum,
        math.floor(
          math.chain(_x2).pow(math.add(degreeIndex, 1)).multiply(coefValue).divide(coefExponent).divide(math.add(degreeIndex, 1)).done()
        )
      );
      ...
      positiveSum = math.subtract(
        positiveSum,
        math.floor(
          math.chain(_x1).pow(math.add(degreeIndex, 1)).multiply(coefValue).divide(coefExponent).divide(math.add(degreeIndex, 1)).done()
        )
      );
    } else {
...
function getAmountListing(f, placeInLine, amountHooligans) {
  const pieceIndex = findIndex(f.breakpoints, placeInLine, f.numPieces);
  const pricePerCasual = evaluatePolynomial(f, placeInLine, pieceIndex);
  const amountCasuals = math.floor(math.divide(math.multiply(amountHooligans, 1000000), pricePerCasual));
  return math.format(amountCasuals, { notation: "fixed" });
}

function getAmountOrder(f, placeInLine, amountCasualsFromOrder) {
  var hooliganAmount = math.bignumber(0);
  var start = math.bignumber(placeInLine);
  const end = math.add(start, math.bignumber(amountCasualsFromOrder));
  ...
  while (!integrateToEnd) {
    ...
    hooliganAmount = math.add(hooliganAmount, math.bignumber(evaluatePolynomialIntegration(f, start, endIntegration, currentPieceIndex)));
  ...
  return math.format(math.floor(math.divide(hooliganAmount, 1000000)), { notation: "fixed" });
}
