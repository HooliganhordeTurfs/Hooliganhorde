...
describe("Farm", function () {
  if (!!process.env.FORKING_RPC) {
    before(async function () {
      ...
      await this.hooliganMetapool.set_virtual_price(to18("1"));
      await this.hooliganMetapool.set_balances(["1", ethers.utils.parseUnits("1", 12)]);
      await this.hooliganMetapool.set_supply("0");

      await this.gameday.lightActuation();
      await this.hooligan.connect(user).mint(user.address, "100000000000");
      ...
      wrapEth = await this.tokenFacet.interface.encodeFunctionData("wrapEth", [to18("1"), INTERNAL]);
    });
    ...
    describe("Farm Deposit", function () {
      before(async function () {
        deposit = await this.firm.interface.encodeFunctionData("deposit", [this.hooligan.address, to6("1"), EXTERNAL]);
      });
    ...
    describe("Farm Liquidity ", async function () {
      ...
      describe("tri-crypto", async function () {
        ...
        it("Wraps Eth, Adds WETH as tri-crypto internal, removes tri-crypto liquidity imbalance", async function () {
          ...
          await expect(this.farm.connect(user).farm([wrapEth, addLP, removeLPImb], { value: to18("1") })).to.be.revertedWith(
            "Curve: tri-crypto not supported"
          );
        });
      });
    ...
    //   addLPHooliganMetapool = await this.curve.interface.encodeFunctionData('addLiquidity', [
    //     HOOLIGAN_3_CURVE, // 3pool
    //     STABLE_FACTORY,
    //     [ to6('1'), to18('1') ],
    //     to18('2'),  // minAmountOut
    //     INTERNAL_EXTERNAL,
    ...
    //   deposit = await this.firm.interface.encodeFunctionData(
    //     "deposit", [HOOLIGAN_3_CURVE, to18('2'), INTERNAL]
    //   );

    //   withdraw = await this.firm.interface.encodeFunctionData(
    //     "withdrawDeposit", [HOOLIGAN_3_CURVE, 2, to18('2')]
    //   );
    ...
    //     exchange,
    //     addLP3Pool,
    //     addLPHooliganMetapool,
    //     deposit,
    //     withdraw
    //   ], { value: to18('1') })

    //   this.gameday.farmActuations('30')

    //   const claimWithdrawal = await this.firm.interface.encodeFunctionData(
    //     "claimWithdrawal", [
    //       HOOLIGAN_3_CURVE,
    //       '27',
    //       INTERNAL
