...
const initializeMerkleTree = async () => {
  ...
  leafNodes1 = data1.map((item) => ethers.utils.solidityKeccak256(["address", "uint256"], [item.user.address, item.amount]));
  ...
  leafNodes2 = data2.map((item) => ethers.utils.solidityKeccak256(["address", "uint256"], [item.user.address, item.amount]));
...
describe("UnripeClaim", function () {
  ...
  it("claim unripe token", async function () {
    const user = data1[0];
    const leaf = ethers.utils.solidityKeccak256(["address", "uint256"], [user.user.address, user.amount]);
    const proof = merkleTree1.getHexProof(leaf);

    const beforeBalance = await this.unripeToken1.balanceOf(user.user.address);
    const tx = await this.unripeClaim.connect(user.user).pick(this.unripeToken1.address, user.amount, proof, EXTERNAL);
    ...
    expect(tx).to.emit(this.unripeClaim, "UnripeTokenClaimed").withArgs(this.unripeToken1.address, user.user.address, user.amount);
  });
  ...
  it("claim unripe token only once", async function () {
    ...
    await this.unripeClaim.connect(user.user).pick(this.unripeToken1.address, user.amount, proof, EXTERNAL);

    await expect(this.unripeClaim.connect(user.user).pick(this.unripeToken1.address, user.amount, proof, EXTERNAL)).to.be.revertedWith(
  ...
  it("claim correct unripe token", async function () {
    ...
    await expect(this.unripeClaim.connect(user.user).pick(this.unripeToken2.address, user.amount, proof, EXTERNAL)).to.be.revertedWith(
  ...
  it("not claim other user's unripe token", async function () {
    ...
    await expect(this.unripeClaim.connect(user2).pick(this.unripeToken1.address, user.amount, proof, EXTERNAL)).to.be.revertedWith(
      "UnripeClaim: invalid proof"
    );
  });
});
