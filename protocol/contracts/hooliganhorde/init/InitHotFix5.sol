...
interface IBs {
    function updateFirm(address account) external;
}
...
contract InitHotFix5 {
    ...
    function init() external {
        ...
        IBs(address(this)).updateFirm(AFFECTED_ADDRESS);

        uint256 expectedRoots = s.s.roots.mul(s.a[AFFECTED_ADDRESS].s.horde).div(s.s.horde);
