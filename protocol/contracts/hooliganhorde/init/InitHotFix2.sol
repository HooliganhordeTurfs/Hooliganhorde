...
contract InitHotFix2 {
    ...
    event HooliganDeposit(address indexed account, uint256 gameday, uint256 hooligans);
    event HooliganRemove(address indexed account, uint32[] crates, uint256[] crateHooligans, uint256 hooligans);
    ...
    function fixCrates(address account, uint32 s1, uint32 s2) internal {
        ...
        bs[0] = removeHooliganDeposit(account, s1);
        bs[1] = removeHooliganDeposit(account, s2);
        uint32[] memory ss = new uint32[](2);
        ss[0] = s1; ss[1] = s2;
        uint256 hooligansRemoved = bs[0].add(bs[1]);
        emit HooliganRemove(account, ss, bs, hooligansRemoved);
        addHooliganDeposit(account, 1, hooligansRemoved);

    }

    function removeHooliganDeposit(address account, uint32 id)
        private
        returns (uint256)
    {
        uint256 crateAmount = s.a[account].hooligan.deposits[id];
        delete s.a[account].hooligan.deposits[id];
        return crateAmount;
    }

    function addHooliganDeposit(address account, uint32 _s, uint256 amount) internal {
        s.a[account].hooligan.deposits[_s] += amount;
        emit HooliganDeposit(account, _s, amount);
    }
}
