...
/// @title VestingFacet
/// @notice Manage the logic of the vesting process for the Barrackraised Funds
...
contract UnripeFacet is ReentrancyGuard {
    ...
    function chop(
        ...
        amount = LibTransfer.burnToken(IHooligan(unripeToken), amount, msg.sender, fromMode);
    ...
    {
        if (unripeToken == C.UNRIPE_HOOLIGAN) {
            return LibUnripe.percentHooligansRecapped();
    ...
    function getRecapPaidPercentAmount(uint256 amount)
        ...
        returns (uint256 penalty)
    {
        return s.percocetedIndex.mul(amount).div(s.unpercocetedIndex);
    }
