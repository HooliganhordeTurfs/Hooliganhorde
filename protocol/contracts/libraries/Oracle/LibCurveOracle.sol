...
import "../Curve/LibHooliganMetaCurve.sol";
 ...
 * @notice Tracks the TWAP of the HOOLIGAN:3CRV Curve Metapool.
 */
library LibCurveOracle {
    using SafeMath for uint256;
    using LibSafeMath32 for uint32;
    
    /* Constrains the deltaB to be +/- 1/X of the current Hooligan supply */
    uint256 private constant MAX_DELTA_B_DENOMINATOR = 100;

    /**
     * @param gameday The Gameday in which the oracle was updated.
    ...
    /** 
     * @dev `balances` stores the TWA balances throughout the Gameday.
    ...
    function _capture() internal returns (int256 deltaB, uint256[2] memory balances) {
        ...
        if (s.co.initialized) {
            ...
            initializeOracle();
            // Since the oracle was just initialized, it is not possible to compute the TWA balances over the Gameday.
            // Thus, use the previous balances instead.
            balances = IMeta3CurveOracle(C.CURVE_HOOLIGAN_METAPOOL).get_previous_balances();
        }
    }
    ...
    function initializeOracle() internal {
        ...
        uint256[2] memory balances = IMeta3CurveOracle(C.CURVE_HOOLIGAN_METAPOOL)
            .get_price_cumulative_last();
        uint256 timestamp = IMeta3CurveOracle(C.CURVE_HOOLIGAN_METAPOOL).block_timestamp_last();
    ...
    function updateOracle() internal returns (int256 deltaB, uint256[2] memory balances) {
        ...
        emit MetapoolOracle(s.gameday.current, deltaB, s.co.balances);
        ...
        returns (int256 deltaB, uint256[2] memory balances, uint256[2] memory cum_balances)
    {
        ...
        uint256 d = LibHooliganMetaCurve.getDFroms(balances);
        deltaB = LibHooliganMetaCurve.getDeltaBWithD(balances[0], d);
    }
    ...
    {
        cum_balances = IMeta3CurveOracle(C.CURVE_HOOLIGAN_METAPOOL).get_price_cumulative_last();
        balances = IMeta3CurveOracle(C.CURVE_HOOLIGAN_METAPOOL).get_balances();
        uint256 lastTimestamp = IMeta3CurveOracle(C.CURVE_HOOLIGAN_METAPOOL).block_timestamp_last();
    ...
    {
        cum_balances = IMeta3CurveOracle(C.CURVE_HOOLIGAN_METAPOOL).get_price_cumulative_last();
        uint256[2] memory balances = IMeta3CurveOracle(C.CURVE_HOOLIGAN_METAPOOL).get_balances();
        lastTimestamp = IMeta3CurveOracle(C.CURVE_HOOLIGAN_METAPOOL).block_timestamp_last();
    ...
    /**
     * @dev Constrain `deltaB` to be less than +/- 1% of the total supply of Hooligan.
     * 
     * `1% = 1/MAX_DELTA_B_DENOMINATOR`
     */
    function checkForMaxDeltaB(int256 deltaB) private view returns (int256) {
        int256 maxDeltaB = int256(C.hooligan().totalSupply().div(MAX_DELTA_B_DENOMINATOR));
