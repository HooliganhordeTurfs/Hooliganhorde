...
library LibMetaCurveConvert {
    ...
    /**
     * @notice Calculate the amount of HOOLIGAN that would exist in a Curve metapool
     * if it were "at peg", i.e. if there was 1 HOOLIGAN per 1 USD of 3CRV.
     * @dev Assumes that `balances[1]` is 3CRV.
     */
    function hooligansAtPeg(uint256[2] memory balances)
        internal
        view
    ...
    function lpToPeg(uint256[2] memory balances, uint256 atPeg) internal view returns (uint256 lp) {
        ...
        uint256[2] memory xp = LibHooliganMetaCurve.getXP(balances);
    ...
    function _toPegWithFee(
        ...
        uint256[2] memory xp = LibHooliganMetaCurve.getXP(balances);
        uint256 new_y = LibHooliganMetaCurve.getXP0(balances[0].sub(amount));
        ...
        dy = LibHooliganMetaCurve.getX0(dy.sub(1));
        uint256 dy_0 = LibHooliganMetaCurve.getX0(xp[0].sub(new_y));
