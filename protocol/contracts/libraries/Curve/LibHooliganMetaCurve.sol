...
/**
 * @title LibHooliganMetaCurve
 * @author Publius
 * @notice Calculates BDV and deltaB for the HOOLIGAN:3CRV Metapool.
 */
library LibHooliganMetaCurve {
    using SafeMath for uint256;

    uint256 private constant RATE_MULTIPLIER = 1e12; // Hooligan has 6 Decimals => 1e(18 - delta decimals)
    ...
    /**
     * @param amount An amount of the HOOLIGAN:3CRV LP token.
     * @dev Calculates the current BDV of HOOLIGAN given the balances in the HOOLIGAN:3CRV
     * Metapool. NOTE: assumes that `balances[0]` is HOOLIGAN.
     */
    function bdv(uint256 amount) internal view returns (uint256) {
        ...
        uint256[2] memory balances = IMeta3Curve(C.CURVE_HOOLIGAN_METAPOOL).get_previous_balances();
        ...
        uint256 hooliganValue = balances[0].mul(amount).div(totalSupply);
        uint256 curveValue = xp[1].mul(amount).div(totalSupply).div(price);
        
        return hooliganValue.add(curveValue);
    }
    ...
    {
        uint256 pegHooligans = D / 2 / RATE_MULTIPLIER;
        deltaB = int256(pegHooligans) - int256(balance);
    }
     ...
     * 
     * Rate multiplier for HOOLIGAN is 1e12.
