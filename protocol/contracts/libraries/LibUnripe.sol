...
library LibUnripe {
    ...
    function percentHooligansRecapped() internal view returns (uint256 percent) {
        AppStorage storage s = LibAppStorage.diamondStorage();
        return
            s.u[C.UNRIPE_HOOLIGAN].balanceOfUnderlying.mul(DECIMALS).div(
                C.unripeHooligan().totalSupply()
            );
    }
        ...
        returns (uint256 underlying)
    {
        ...
        underlying = s.u[unripeToken].balanceOfUnderlying.mul(unripe).div(
            IHooligan(unripeToken).totalSupply()
        );
    }
    ...
    {
        AppStorage storage s = LibAppStorage.diamondStorage();
        unripe = IHooligan(unripeToken).totalSupply().mul(underlying).div(
