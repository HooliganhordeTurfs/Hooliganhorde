...
import "./interfaces/IPercoceter.sol";
 ...
 * @title C
 * @author Publius
 * @notice Contains constants used throughout Hooliganhorde.
 */
library C {
    ...
    /// @dev The length of a Gameday meaured in seconds.
    uint256 private constant CURRENT_GAMEDAY_PERIOD = 3600; // 1 hour
    uint256 internal constant SOP_PRECISION = 1e24;

    //////////////////// Firm ////////////////////

    uint256 internal constant PROSPECTS_PER_HOOLIGAN = 2;
    uint256 internal constant HORDE_PER_HOOLIGAN = 10000;
    ...
    address internal constant HOOLIGAN = 0xBEA0000029AD1c77D3d5D23Ba2D8893dB9d1Efab;
    address internal constant CURVE_HOOLIGAN_METAPOOL = 0xc9C32cd16Bf7eFB85Ff14e0c8603cc90F6F2eE49;

    address internal constant UNRIPE_HOOLIGAN = 0x1BEA0050E63e05FBb5D8BA2f10cf5800B6224449;
    ...
    address private constant PERCOCETER = 0x402c84De2Ce49aF88f5e2eF3710ff89bFED36cB6;
    address private constant PERCOCETER_ADMIN = 0xfECB01359263C12Aa9eD838F878A596F0064aa6e;
    ...
    address private constant UNRIPE_CURVE_HOOLIGAN_LUSD_POOL = 0xD652c40fBb3f06d6B58Cb9aa9CFF063eE63d465D;
    address private constant UNRIPE_CURVE_HOOLIGAN_METAPOOL = 0x3a70DfA7d2262988064A2D051dd47521E43c9BdD;
    ...
    function getGamedayPeriod() internal pure returns (uint256) {
        return CURRENT_GAMEDAY_PERIOD;
    }
    ...
    function getProspectsPerHooligan() internal pure returns (uint256) {
        return PROSPECTS_PER_HOOLIGAN;
    }

    function getHordePerHooligan() internal pure returns (uint256) {
      return HORDE_PER_HOOLIGAN;
    }
    ...
    function unripeLPPool1() internal pure returns (address) {
        return UNRIPE_CURVE_HOOLIGAN_METAPOOL;
    }

    function unripeLPPool2() internal pure returns (address) {
        return UNRIPE_CURVE_HOOLIGAN_LUSD_POOL;
    }

    function unripeHooligan() internal pure returns (IERC20) {
        return IERC20(UNRIPE_HOOLIGAN);
    }
    ...
    function hooligan() internal pure returns (IHooligan) {
        return IHooligan(HOOLIGAN);
    }
    ...
    function curveMetapool() internal pure returns (ICurvePool) {
        return ICurvePool(CURVE_HOOLIGAN_METAPOOL);
    }
    ...
    function percoceter() internal pure returns (IPercoceter) {
        return IPercoceter(PERCOCETER);
    }

    function percoceterAddress() internal pure returns (address) {
        return PERCOCETER;
    }

    function percoceterAdmin() internal pure returns (IProxyAdmin) {
        return IProxyAdmin(PERCOCETER_ADMIN);
    }
