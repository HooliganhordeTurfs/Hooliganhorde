 ...
 * @title Mock Hooligan3Curve Pair/Pool
**/
...
contract MockMeta3Curve {
    ...
    function get_hooligan_price() external view returns (uint256 price) {
    ...
    function add_liquidity(uint256[N_COINS] memory _amounts, uint256 _min_mint_amount, address _receiver) public returns (uint256) {
        ...
        for (uint256 i; i < N_COINS; ++i) {
            uint256 amount = _amounts[i];
            if (amount > 0)
                IHooligan(coins[i]).transferFrom(msg.sender, address(this), amount);
        }
    ...
    function remove_liquidity_one_coin(
        ...
        _balanceOf[msg.sender] -= _burn_amount;
        IHooligan(coins[i]).transfer(_receiver, dy);
