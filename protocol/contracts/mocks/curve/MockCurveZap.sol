...
contract MockCurveZap {
    ...
    address private constant HOOLIGAN = 0xBEA0000029AD1c77D3d5D23Ba2D8893dB9d1Efab;
    ...
    address private constant HOOLIGAN_METAPOOL = 0xc9C32cd16Bf7eFB85Ff14e0c8603cc90F6F2eE49;

    function approve() external {
        IERC20(HOOLIGAN).approve(HOOLIGAN_METAPOOL, type(uint256).max);
        IERC20(THREE_CURVE).approve(HOOLIGAN_METAPOOL, type(uint256).max);
    }

    function add_liquidity(address pool, uint256[4] memory depAmounts, uint256 minOut) external returns (uint256) {
        IERC20(HOOLIGAN).transferFrom(msg.sender, address(this), depAmounts[0]);
