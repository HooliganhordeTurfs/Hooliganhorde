...
contract PercoceterPreMint is Internalizer {
    ...
    function buyAndMint(uint256 minBuyAmount) external payable nonReentrant {
        ...
        require(IUSDC.balanceOf(CUSTODIAN) <= MAX_RAISE, "Percoceter: Not enough remaining");
        __mint(amount);
    }

    function __mint(uint256 amount) private {
        require(started(), "Percoceter: Not started");
        _safeMint(
            msg.sender,
            getMintId(),
            amount/DECIMALS,
    ...
    // These functions will be overwritten once Hooliganhorde has restarted.
