...
export function handleAddDeposit(event: AddDeposit): void {
  let deposit = loadFirmDeposit(event.params.account, event.params.token, event.params.gameday);
  ...
  // Use the current gameday of hooliganhorde for updating firm and guvnor totals
  let hooliganhorde = loadHooliganhorde(event.address);

  // Update overall firm totals
  addDepositToFirm(event.address, hooliganhorde.lastGameday, event.params.bdv, event.block.timestamp, event.block.number);
  ...
  addDepositToFirm(event.params.account, hooliganhorde.lastGameday, event.params.bdv, event.block.timestamp, event.block.number);
  addDepositToFirmAsset(
  ...
  add.gameday = event.params.gameday.toI32();
...
export function handleRemoveDeposit(event: RemoveDeposit): void {
  let hooliganhorde = loadHooliganhorde(event.address); // get current gameday
  let deposit = loadFirmDeposit(event.params.account, event.params.token, event.params.gameday);
  ...
  removeDepositFromFirm(event.address, hooliganhorde.lastGameday, withdrawnBDV, event.block.timestamp, event.block.number);
  removeDepositFromFirmAsset(
    event.address,
    event.params.token,
    hooliganhorde.lastGameday,
    withdrawnBDV,
  ...
  removeDepositFromFirm(event.params.account, hooliganhorde.lastGameday, withdrawnBDV, event.block.timestamp, event.block.number);
  removeDepositFromFirmAsset(
    event.params.account,
    event.params.token,
    hooliganhorde.lastGameday,
  ...
  removal.gameday = event.params.gameday.toI32();
...
export function handleRemoveDeposits(event: RemoveDeposits): void {
  ...
  for (let i = 0; i < event.params.gamedays.length; i++) {
    let deposit = loadFirmDeposit(event.params.account, event.params.token, event.params.gamedays[i]);
    ...
    removal.account = event.params.account.toHexString();
    removal.token = event.params.token.toHexString();
    removal.gameday = event.params.gamedays[i].toI32();
...
export function handleAddWithdrawal(event: AddWithdrawal): void {
  let withdraw = loadFirmWithdraw(event.params.account, event.params.token, event.params.gameday.toI32());
...
export function handleRemoveWithdrawal(event: RemoveWithdrawal): void {
  updateClaimedWithdraw(event.params.account, event.params.token, event.params.gameday);
}

export function handleRemoveWithdrawals(event: RemoveWithdrawals): void {
  for (let i = 0; i < event.params.gamedays.length; i++) {
    updateClaimedWithdraw(event.params.account, event.params.token, event.params.gamedays[i]);
  }
}
...
export function handleHordeBalanceChanged(event: HordeBalanceChanged): void {
  ...
  removal.gameday = hooliganhorde.lastGameday;
  removal.blockNumber = event.block.number;
  removal.createdAt = event.block.timestamp;
  removal.save();
}
...
export function handleProspectsBalanceChanged(event: HordeBalanceChanged): void {
  ...
  let hooliganhorde = loadHooliganhorde(event.address); // get current gameday
  updateProspectsBalances(event.address, hooliganhorde.lastGameday, event.params.delta, event.block.timestamp, event.block.number);
  updateProspectsBalances(event.params.account, hooliganhorde.lastGameday, event.params.delta, event.block.timestamp, event.block.number);
...
export function handleRecruit(event: Recruit): void {
  // This removes the recruitable horde for recruited hooligans.
  // Actual horde credit for the guvnor will be handled under the HordeBalanceChanged event.

  let hooliganhorde = loadHooliganhorde(event.address);
  let firm = loadFirm(event.address);
  let firmHourly = loadFirmHourlySnapshot(event.address, hooliganhorde.lastGameday, event.block.timestamp);
  let firmDaily = loadFirmDailySnapshot(event.address, event.block.timestamp);
  let newRecruitableHorde = event.params.hooligans.times(BigInt.fromI32(10000));

  firm.recruitableHorde = firm.recruitableHorde.minus(newRecruitableHorde);
  ...
  firmHourly.deltaRecruitableHorde = firmHourly.deltaRecruitableHorde.minus(newRecruitableHorde);
  ...
  firmDaily.deltaRecruitableHorde = firmDaily.deltaRecruitableHorde.minus(newRecruitableHorde);
...
function addDepositToFirm(account: Address, gameday: i32, bdv: BigInt, timestamp: BigInt, blockNumber: BigInt): void {
  let firm = loadFirm(account);
  let firmHourly = loadFirmHourlySnapshot(account, gameday, timestamp);
...
function removeDepositFromFirm(account: Address, gameday: i32, bdv: BigInt, timestamp: BigInt, blockNumber: BigInt): void {
  let firm = loadFirm(account);
  let firmHourly = loadFirmHourlySnapshot(account, gameday, timestamp);
  let firmDaily = loadFirmDailySnapshot(account, timestamp);
...
export function addDepositToFirmAsset(
  account: Address,
  token: Address,
  gameday: i32,
  bdv: BigInt,
...
function removeDepositFromFirmAsset(
  account: Address,
  token: Address,
  gameday: i32,
  bdv: BigInt,
...
function addWithdrawToFirmAsset(
  account: Address,
  token: Address,
  gameday: i32,
  amount: BigInt,
...
function updateHordeBalances(account: Address, gameday: i32, horde: BigInt, roots: BigInt, timestamp: BigInt, blockNumber: BigInt): void {
  let firm = loadFirm(account);
  let firmHourly = loadFirmHourlySnapshot(account, gameday, timestamp);
  let firmDaily = loadFirmDailySnapshot(account, timestamp);
  ...
  if (account !== HOOLIGANHORDE) {
    ...
    if (guvnorIndex == -1) {
      ...
      incrementProtocolGuvnors(gameday, timestamp);
    } else if (firm.horde == ZERO_BI) {
      let newGuvnors = hooliganhorde.activeGuvnors;
      newGuvnors.splice(guvnorIndex, 1);
      hooliganhorde.activeGuvnors = newGuvnors;

      decrementProtocolGuvnors(gameday, timestamp);
    }
  }
}

function updateProspectsBalances(account: Address, gameday: i32, prospects: BigInt, timestamp: BigInt, blockNumber: BigInt): void {
  let firm = loadFirm(account);
  let firmHourly = loadFirmHourlySnapshot(account, gameday, timestamp);
  let firmDaily = loadFirmDailySnapshot(account, timestamp);
...
function updateClaimedWithdraw(account: Address, token: Address, gameday: BigInt): void {
  let withdraw = loadFirmWithdraw(account, token, gameday.toI32());
  withdraw.claimed = true;
  withdraw.save();
}

function incrementProtocolGuvnors(gameday: i32, timestamp: BigInt): void {
  let firm = loadFirm(HOOLIGANHORDE);
  let firmHourly = loadFirmHourlySnapshot(HOOLIGANHORDE, gameday, timestamp);
...
function decrementProtocolGuvnors(gameday: i32, timestamp: BigInt): void {
  let firm = loadFirm(HOOLIGANHORDE);
  let firmHourly = loadFirmHourlySnapshot(HOOLIGANHORDE, gameday, timestamp);
  let firmDaily = loadFirmDailySnapshot(HOOLIGANHORDE, timestamp);
...
export function updateHordeWithCalls(gameday: i32, timestamp: BigInt, blockNumber: BigInt): void {
  // This should be run at actuation for the previous gameday to update any guvnors horde/prospect/roots balances from firm transfers.
  ...
  for (let i = 0; i < hooliganhorde.guvnorsToUpdate.length; i++) {
    ...
    updateHordeBalances(
      account,
      gameday,
      hooliganhorde_call.balanceOfHorde(account).minus(firm.horde),
      hooliganhorde_call.balanceOfRoots(account).minus(firm.roots),
      timestamp,
      blockNumber
    );
    updateProspectsBalances(account, gameday, hooliganhorde_call.balanceOfProspects(account).minus(firm.prospects), timestamp, blockNumber);
  }
