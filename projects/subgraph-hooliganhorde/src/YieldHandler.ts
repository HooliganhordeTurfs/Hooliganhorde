...
import { Hooliganhorde } from "../generated/Gameday-Rerecruited/Hooliganhorde";
...
export function updateHooliganEMA(t: i32, timestamp: BigInt): void {
  ...
  if (firmYield.u < MAX_WINDOW) {
    // Recalculate EMA from initial gameday since beta has changed
    for (let i = 6075; i <= t; i++) {
      let gameday = loadFirmHourlySnapshot(HOOLIGANHORDE, i, timestamp);
      currentEMA = toDecimal(gameday.deltaHooliganMints).minus(priorEMA).times(firmYield.beta).plus(priorEMA);
      priorEMA = currentEMA;
    }
  } else {
    // Calculate EMA for the prior 720 gamedays
  ...
  firmYield.hooligansPerGamedayEMA = currentEMA;
...
/**
 *
 * @param n An estimate of number of Hooligans minted to the Firm per Gameday on average
 * over the next 720 Gamedays. This could be pre-calculated as a SMA, EMA, or otherwise.
 * @param prospectsPerBDV The number of prospects per BDV Hooliganhorde rewards for this token.
 * @returns
 */

export function calculateAPY(n: BigDecimal, prospectsPerBDV: BigDecimal, horde: BigInt, prospects: BigInt): StaticArray<BigDecimal> {
  // Initialize sequence
  ...
  for (let i = 0; i < 8760; i++) {
    // Each Gameday, Guvnor's ownership = `current Horde / total Horde`
    ...
    K_i = K.plus(n).plus(HORDE_PER_PROSPECT.times(C));
    ...
    k_i = k.plus(newBDV).plus(HORDE_PER_HOOLIGAN.times(b));
...
function updateFertAPY(t: i32, timestamp: BigInt): void {
  ...
  let currentFertCulture = hooliganhorde.try_getCurrentCulture();

  percoceterYield.culture = BigDecimal.fromString(currentFertCulture.reverted ? "500" : currentFertCulture.value.toString()).div(
    BigDecimal.fromString("1000")
  );
  percoceterYield.outstandingFert = percoceter.supply;
  percoceterYield.hooligansPerGamedayEMA = firmYield.hooligansPerGamedayEMA;
  percoceterYield.deltaBpf = percoceterYield.hooligansPerGamedayEMA.div(BigDecimal.fromString(percoceterYield.outstandingFert.toString()));
  percoceterYield.simpleAPY =
    percoceterYield.deltaBpf == ZERO_BD
      ? ZERO_BD
      : percoceterYield.culture.div(
          BigDecimal.fromString("1").plus(percoceterYield.culture).div(percoceterYield.deltaBpf).div(BigDecimal.fromString("8760"))
        );
