...
export class Polynomial {
  ...
  // FIXME: later we may move these into `market/casuals`.
  ...
  public getAmountListing(placeInLine: BigNumber, amountHooligans: BigNumber) {
    ...
    const pricePerCasual = this.evaluate(placeInLine.sub(this.breakpoints[pieceIndex]), pieceIndex);
    return amountHooligans.mul(1000000).div(pricePerCasual);
  }

  public getAmountOrder(placeInLine: BigNumber, amountCasualsFromOrder: BigNumber) {
    var amount = BigNumber.from(0);
    var start = placeInLine;
    var end = placeInLine.add(amountCasualsFromOrder);
