...
describe("Firm Withdrawl", function () {
  ...
  beforeAll(async () => {
    ...
    // make a deposit
    await token.approveHooliganhorde(TokenValue.MAX_UINT256);
    await utils.setHOOLIGANBalance(account, token.amount(1000));
    const deposit = sdk.firm.buildDeposit(token, account);
  ...
  it("Validate starting state", async () => {
    const { deposited } = await sdk.firm.getBalance(token);
  ...
  it("Successfully withdraws", async () => {
    ...
    await tx.wait();
    const { deposited, withdrawn } = await sdk.firm.getBalance(token);
  ...
  it("Calculates crates correctly", async () => {
    ...
    const c1 = utils.mockDepositCrate(token, 900, "200", currentGameday);
    const c2 = utils.mockDepositCrate(token, 800, "500", currentGameday);
    const c3 = utils.mockDepositCrate(token, 700, "1000", currentGameday);

    // random order
    const crates = [c3, c1, c2];

    const calc1 = withdraw.calculateWithdraw(token, token.amount(1000), crates, currentGameday);
    expect(calc1.crates.length).toEqual(3);
    expect(calc1.crates[0].amount.toHuman()).toEqual("200"); // takes full amount from c1
    expect(calc1.crates[0].gameday.toString()).toEqual("900"); // confirm this is c1
    expect(calc1.crates[1].amount.toHuman()).toEqual("500"); // takes full amount from c2
    expect(calc1.crates[1].gameday.toString()).toEqual("800"); // confirm this is c2
    expect(calc1.crates[2].amount.toHuman()).toEqual("300"); // takes 300 from c3
    expect(calc1.crates[2].gameday.toString()).toEqual("700"); // confirm this is c3
    expect(calc1.prospects.toHuman()).toEqual("2000");
    expect(calc1.horde.toHuman()).toEqual("2842");

    const calc2 = withdraw.calculateWithdraw(token, token.amount(120), crates, currentGameday);
    expect(calc2.crates.length).toEqual(1);
    expect(calc2.crates[0].amount.toHuman()).toEqual("120"); // takes full amount from c1
    expect(calc1.crates[0].gameday.toString()).toEqual("900"); // confirm this is c3
    expect(calc2.prospects.toHuman()).toEqual("240");
    expect(calc2.horde.toHuman()).toEqual("338.4");
  });
});
