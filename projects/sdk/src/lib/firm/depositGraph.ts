...
export const getDepositGraph = (sdk: HooliganhordeSDK): Graph => {
  ...
  {
    /**
     * Handle Other swaps to HOOLIGAN
     */
    ...
    graph.setEdge("USDC", "HOOLIGAN", {
      build: (_: string, from: FarmFromMode, to: FarmToMode) => sdk.farm.presets.usdc2hooligan(from, to),
      from: "USDC",
      to: "HOOLIGAN"
    });

    graph.setEdge("DAI", "HOOLIGAN", {
      build: (_: string, from: FarmFromMode, to: FarmToMode) => sdk.farm.presets.dai2hooligan(from, to),
      from: "DAI",
      to: "HOOLIGAN"
    });
  }
