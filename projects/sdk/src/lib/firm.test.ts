...
import { TokenFirmBalance } from "./firm/types";
import { calculateGrownHorde, parseWithdrawalCrates } from "./firm/utils";
...
describe("Firm Balance loading", () => {
  ...
  describe("Grown Horde calculations", () => {
    ...
    it("returns zero when deltaGamedays = 0", () => {
      expect(calculateGrownHorde(6074, 6074, prospects).toHuman()).to.eq("0");
    });
    it("throws if currentGameday < depositGameday", () => {
      expect(() => calculateGrownHorde(5000, 6074, prospects).toHuman()).to.throw();
    });
    it("works when deltaGamedays > 0", () => {
      // 1 prospect grows 1/10_000 HORDE per Gameday
      expect(calculateGrownHorde(6075, 6074, prospects).toHuman()).to.eq((1 / 10_000).toString());
      expect(calculateGrownHorde(6075, 6074, prospects.mul(10)).toHuman()).to.eq((10 / 10_000).toString());
      expect(calculateGrownHorde(6076, 6074, prospects).toHuman()).to.eq((2 / 10_000).toString());
      expect(calculateGrownHorde(6076, 6074, prospects.mul(10)).toHuman()).to.eq((20 / 10_000).toString());
    });
  });
});
