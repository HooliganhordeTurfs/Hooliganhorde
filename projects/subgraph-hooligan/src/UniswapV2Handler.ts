...
import { Swap, Sync, UniswapV2Pair } from "../generated/HooliganUniswapV2Pair/UniswapV2Pair";
import { updateHooliganSupplyPegPercent, updateHooliganValues } from "./utils/Hooligan";
import { HOOLIGAN_ERC20_V1, WETH, WETH_USDC_PAIR } from "./utils/Constants";
...
export function handleSwap(event: Swap): void {
  ...
  // Token 0 is WETH and Token 1 is HOOLIGAN
  ...
  updateHooliganValues(HOOLIGAN_ERC20_V1.toHexString(), event.block.timestamp, pool.lastPrice, ZERO_BI, ZERO_BI, usdVolume, ZERO_BD);
}
...
export function handleSync(event: Sync): void {
  ...
  let hooliganBalance = toDecimal(reserves.value.value1);
  ...
  let deltaHooligans = toBigInt(wethBalance.times(weth.lastPriceUSD).minus(hooliganBalance), 6);
  ...
  let currentHooliganPrice = wethBalance.times(weth.lastPriceUSD).div(hooliganBalance);

  updatePoolPrice(event.address.toHexString(), event.block.timestamp, event.block.number, currentHooliganPrice);
  ...
  updateHooliganValues(
    HOOLIGAN_ERC20_V1.toHexString(),
    event.block.timestamp,
    currentHooliganPrice,
    ZERO_BI,
    ZERO_BI,
    ZERO_BD,
    deltaLiquidityUSD
