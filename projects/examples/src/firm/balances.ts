import { DataSource, Token, TokenValue } from "@xblackfury/sdk";
...
async function showSummary(account: string) {
  ...
  const recruitableProspects = (await sdk.firm.getRecruitableProspects(account)).toHuman();
  ...
  const earned = [
    ...
    ["Earnings", "", "", "", "", ""],
    ["Earned Hooligans", "Earned Horde", "Recruitable Prospects", "Grown Horde", "Revitalized Horde", "Revitalized Prospects"],
    [earnedHooligans, earnedHorde, recruitableProspects, grownHorde, revHorde, revProspects]
  ];
