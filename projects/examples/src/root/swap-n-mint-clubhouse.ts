import { FarmFromMode, FarmToMode, TokenValue, TestUtils, Clipboard, DataSource, Token, Workflow, ERC20Token } from "@xblackfury/sdk";
import { ERC20 } from "@xblackfury/sdk/dist/types/constants/generated";
...
export async function roots_via_swap(inputToken: Token, spender: string, amount: TokenValue) {
  ...
  const clubhouseFarm = sdk.farm.create<{ permit: any }>("ClubhouseMint", "clubhouse");
  ...
  pipe.add(
    ...
    },
    {
      ...
      skip: (amountInStep) =>
        inputToken.hasEnoughAllowance(sdk.contracts.pipeline.address, sdk.contracts.hooliganhorde.address, amountInStep)
    }
  );
  ...
  pipe.add(
    ...
    {
      skip: (amountInStep) =>
        depositToken.hasEnoughAllowance(sdk.contracts.pipeline.address, sdk.contracts.hooliganhorde.address, amountInStep)
    }
  );
  ...
  pipe.add(
    ...
    //    the amount deposited (but not the corresponding `gameday`, `bdv`, etc.), we "mock"
    //    the deposit transfer struct using the current gameday.
    ...
    async function mintRoots(amountInStep, context) {
      const [currentGameday, estimatedDepositBDV] = await Promise.all([
        sdk.codex.getGameday(),
      ...
      const estimate = await sdk.root.estimateRoots(
        ...
        [
          ...
          // Note that the gameday of deposit is expected to equal the current gameday
          // since we're depositing and minting in one transaction.
          sdk.firm.makeDepositCrate(
            depositToken,
            currentGameday,
