...
describe('estimation', () => {
  /// NOTE: requires rerecruited hooliganhorde
  ...
  it.skip('estimates: 1 ETH -> X HOOLIGAN', async () => {
    const result = await Farm.estimate(farm.buyHooligans(), [
      ethers.utils.parseUnits('1', 18),
    ]); //
    console.log(
      `1 ETH -> X HOOLIGAN: ${ethers.utils.formatUnits(result.amountOut, 6)}`
    ); // 1000306788
  ...
  it.skip('estimates: X ETH <- 1000 HOOLIGAN', async () => {
    ...
    console.log(
      `X ETH <- 1000 HOOLIGAN: ${ethers.utils.formatUnits(result.amountOut, 18)}`
    ); // 997470693756958276
    expect(
