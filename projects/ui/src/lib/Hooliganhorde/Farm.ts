 ...
 *  - I want to buy Casuals from a Casual Listing. It's going to cost me 1,000 Hooligans but I only have ETH.
...
export default class Farm {
  ...
  buyHooligans = (_initialFromMode?: FarmFromMode) => [
  ...
  pair = {
    WETH_HOOLIGAN: (
      ...
      const Hooligan = getChainConstant(
        HOOLIGAN,
        this.provider.network.chainId
      ).address;
  ...
  wrapEth =
    ...
    async (_amountInStep: ethers.BigNumber) => {
      ...
      return {
        ...
        decode: (data: string) =>
          this.contracts.hooliganhorde.interface.decodeFunctionData(
            'wrapEth',
            data
          ),
      };
  ...
  transferToken =
    ...
    async (_amountInStep: ethers.BigNumber) => {
      ...
      return {
        ...
        encode: (_: ethers.BigNumber) =>
          this.contracts.hooliganhorde.interface.encodeFunctionData(
            'transferToken',
            [
              _tokenIn, //
              _recipient, //
