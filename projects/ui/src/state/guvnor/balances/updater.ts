...
export const useFetchGuvnorBalances = () => {
      ...
      if (account && erc20TokenMap) {
        ...
        const promises = Promise.all([
          ...
          hooliganhorde
            ...
            .then((result) => {
              console.debug(
                '[guvnor/balances/updater]: getAllBalances = ',
                result
              );
              return result;
            })
        ...
        //   // from the standard getAllBalances call.
        //   multiCall.getEthBalance(account),
        //   wrap(hooliganhordeRerecruited).getAllBalances(account, erc20Addresses)
        // ];

        console.debug(
          `[guvnor/updater/useFetchBalances] FETCH: balances (account = ${account})`
        );
  ...
  }, [dispatch, hooliganhorde, Eth, erc20TokenMap, account]);
