...
const RewardsForm: React.FC<RewardsFormProps> = ({ open, children }) => {
  ...
  const [fetchGuvnorFirm] = useFetchGuvnorFirm();
  ...
  const estimateGas = useCallback(async () => {
    ...
    const _calls: ClaimCalls = {
      ...
      [ClaimRewardsAction.RECRUIT_AND_MOW]: {
        estimateGas: () => hooliganhorde.estimateGas.recruit(),
        execute: () => hooliganhorde.recruit(),
      ...
      [ClaimRewardsAction.ENROOT_AND_MOW]: {
        estimateGas: () =>
          hooliganhorde.estimateGas.farm([
            // RECRUIT_AND_MOW
            hooliganhorde.interface.encodeFunctionData('recruit', undefined),
            // ENROOT_AND_MOW
            ...enrootData,
          ]),
        execute: () =>
...
export type RewardsFormContentOption = {
  title: 'Mow' | 'Recruit' | 'Enroot' | string;
  description: string;
