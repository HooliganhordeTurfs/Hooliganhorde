...
import { HooliganhordeSDK, ERC20Token, StepGenerator, Token } from '@xblackfury/sdk';
              ...
              <TxnAccordion defaultExpanded={false}>
                <TxnPreview
                  actions={[
                    ...
                    token.equals(sdk.tokens.HOOLIGAN_CRV3_LP) &&
...
const ClaimPropProvider: FC<{
  ...
  const isHooligan = sdk.tokens.HOOLIGAN.equals(token);
  ...
  const initialValues: ClaimFormValues = useMemo(
    () => ({
      ...
      farmActions: {
        ...
        primary: undefined,
        secondary: undefined,
        exclude: isHooligan ? [FormTxn.CLAIM] : undefined,
      },
    }),
    [token, claimableBalance, isHooligan]
  );
  ...
  const onSubmit = useCallback(
    async (
      ...
      try {
        ...
        const gamedays = crates.map((crate) => crate.gameday.toString());

        const claimTxn = new ClaimFarmStep(sdk, tokenIn, gamedays);
