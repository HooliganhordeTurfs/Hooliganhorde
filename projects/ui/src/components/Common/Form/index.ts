...
export type FormStateWithTurfSelect = FormState & {
  turf?: BigNumber;
};
...
/**
 * Fragment: A single Token stored within a form.
 * NOTE: Duplicated from FromTokenState but to use types from @xblackfury/sdk
 */
...
export type TurfFragment = {
  /** The absolute index of the turf. @decimals 6 */
  index: string | null;
...
export type TurfSettingsFragment = {
  /** Let the Guvnor select the exact range from which their
   * Casuals are being transferred, sold, etc. */
...
export type FormTxnsFormState = {
  /**
   * actions added in conjunction to an arbitrary txn (e.g. deposit, convert, draft, etc).
   */
  farmActions: FormTxnBundlerInterface;
};

export type ClaimHooligansFormState = {
