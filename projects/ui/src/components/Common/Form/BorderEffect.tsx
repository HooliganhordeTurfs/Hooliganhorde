...
> = ({ children, enabled = true, fullWidth = false, disabled = false }) => {
  ...
  const actions = useMemo(() => {
    const handleMouseOver = () => {
      ...
      ref.current.style.boxShadow = `inset 0 0 0 1px ${HooliganhordePalette.textBlue}`;
    };

    const handleMouseLeave = () => {
      if (!ref.current || activeRef.current || disabled) return;
      ref.current.style.boxShadow = `inset 0 0 0 1px ${HooliganhordePalette.inputGrey}`;
    };
  ...
  useEffect(() => {
    ...
    if (disabled) {
      ref.current.style.borderColor = HooliganhordePalette.inputGrey;
    }
  }, [enabled, disabled]);
  ...
  return !enabled ? (
    ...
    <Box
      ...
      sx={{
        borderRadius: 1,
        border: 'none',
        boxShadow: `inset 0 0 0 1px ${HooliganhordePalette.inputGrey}`,
