...
const MultiLineChartInner: React.FC<Props> = (props) => {
  ...
  const handlePointerMove = useCallback(
    (
      ...
      const gameday = pointerData.length ? pointerData[0].gameday : undefined;
      onCursor?.(gameday, getDisplayValue(pointerData), pointerData[0].date);
    },
    [
      containerBounds,
      getPointerValue,
  ...
  const [tickGamedays, tickDates] = useMemo(() => {
