...
const chartColors = HooliganhordePalette.theme.spring.chart;
const defaultChartStyles: ChartMultiStyles = {
  0: {
    stroke: HooliganhordePalette.theme.spring.hooliganhordeGreen,
 ...
 * where data.gameday = k, such that k is the gameday being searched.
 * If more than value satisfies data.gameday = k, the lowest index
 * of the search results will be used
 */
const bisectGameday = bisector<BaseDataPoint, number>((d) => d.gameday).left;

/**
 * access 'gameday' property given from a BaseDataPoint
 */
const getX = (d: BaseDataPoint) => d.gameday;
...
const getPointerValue = (
  ...
  return scales.map((scale, i) => {
    ...
    const index = bisectGameday(data, x0, 1);
...
/**
 * Adds a horizontal line at Gameday 6074.
 */
export const ExploitLine = (props: ChartChildParams) => {
  if (!props.scales.length) return null;
  const exploitGameday = props.scales[0].xScale(6074) as number;
  return exploitGameday ? (
    <Line
      from={{ x: exploitGameday, y: props.dataRegion.yTop }}
      to={{ x: exploitGameday, y: props.dataRegion.yBottom }}
