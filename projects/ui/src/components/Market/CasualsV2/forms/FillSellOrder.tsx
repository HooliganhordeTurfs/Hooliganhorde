...
const FillSellOrder: React.FC<{}> = () => {
  ...
  useEffect(() => {
    if (selected) {
      setFulfillAmount(selected.amount.times(selected.pricePerCasual));
    }
      ...
      {selected && (
        <>
          <InfoRow label="PRICE" infoVariant="caption" labelVariant="caption">
            {`${displayFullBN(selected?.pricePerCasual || ZERO_BN, 2)} HOOLIGAN`}
          </InfoRow>
          <InfoRow
            label="AMOUNT LISTED"
            infoVariant="caption"
          ...
          >
            {`${displayFullBN(selected?.remainingAmount || ZERO_BN, 2)} CASUALS`}
          </InfoRow>
          <Divider />
          <Stack spacing={0.8}>
            <AtomInputField
                ...
                endAdornment: (
                  <InputAdornment position="start">
                    <Typography
                      ...
                      sx={{ ml: 0.2, mt: 0.2 }}
                    >
                      CASUALS @ {displayBN(selected?.placeInLine || ZERO_BN)} IN
                      LINE
                    </Typography>
                  </InputAdornment>
                ),
