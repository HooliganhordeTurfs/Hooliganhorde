...
import EntityIcon from '~/components/Market/CasualsV2/Common/EntityIcon';
...
// Calculates a circle radius between MIN_RADIUS and MAX_RADIUS based on the given turfSize
// Uses log-scale stretching to give relative scale among large maxTurfSize values
const MIN_RADIUS = 2;
const MAX_RADIUS = 6;
const calculateCircleRadius = (
  ...
  const logTurfSize = Math.log(turfSize) > 0 ? Math.log(turfSize) : 0;
  const ratio = logTurfSize / Math.log(maxTurfSize);
  ...
  const orderPositions: CirclePosition[] = useMemo(
    () =>
      orders.map((order) => ({
        ...
        // y position is price per casual
        y: yScale(order.pricePerCasual.toNumber()),
        // radius is turf size
        radius: 4,
      })),
  ...
  const listingPositions: CirclePosition[] = useMemo(
    () =>
      listings.map((listing) => ({
        ...
        // y position is price per casual
        y: yScale(listing.pricePerCasual.toNumber()),
        // radius is turf size
    ...
    [listings, maxTurfSize, xScale, yScale]
  );
  ...
  const handleClick = useCallback(() => {
    ...
    // If we're hovering a point, select it
      ...
      if (hoveredPoint.type === 'listing') {
        // handleClickFill(CasualOrderAction.BUY, CasualOrderType.FILL);
        navigate(`/market/buy/${listings[hoveredPoint.index].id}`);
      } else if (hoveredPoint.type === 'order') {
        // handleClickFill(CasualOrderAction.SELL, CasualOrderType.FILL);
    ...
    >
      {(zoom) => (
        <Box sx={{ position: 'relative' }}>
              ...
              <g transform={zoom.toString()}>
                ...
                {cursorPoint?.type === 'listing' && (
                  <ListingPositionLines
                    ...
                    xLabel={listings[cursorPoint.index].pricePerCasual.toFixed(4)}
                ...
                {cursorPoint?.type === 'order' && (
                  <OrderPositionLines
                    x={cursorPoint.coordinate.x}
                    y={cursorPoint.coordinate.y}
                    xLabel={orders[cursorPoint.index].pricePerCasual.toFixed(4)}
          ...
          {/**
           * Show a tooltip with the number of Casuals whenever a point is
              ...
              <TooltipCard>
                <Row gap={0.5}>
                  ...
                  {cursorPoint.type === 'listing'
                    ...
                    : displayBN(
                        orders[cursorPoint.index].casualAmountRemaining
                      )}{' '}
                  Casuals
                </Row>
              </TooltipCard>
...
// const handleClickFill = useCallback((action: CasualOrderAction, type: CasualOrderType) => {
...
// if (orderAction !== CasualOrderAction.BUY) {
//   setOrderAction(CasualOrderAction.BUY);
// }
// if (orderType !== CasualOrderType.ORDER) {
//   setOrderType(CasualOrderType.ORDER);
// }

// const [orderAction, setOrderAction] = useAtom(casualsOrderActionTypeAtom);
// const [orderType, setOrderType] = useAtom(casualsOrderTypeAtom);
