...
} from '~/state/hooliganhorde/codex/actions';
...
const SettingsDialog: FC<{ open: boolean; onClose?: () => void }> = ({
  ...
  /// Dev Controls
  const setGamedayTimer = useCallback(() => {
  ...
  const exportDepositsCSV = useCallback(() => {
    const rows = Object.keys(firmBalances).reduce(
      (prev, curr) => {
        prev.push(
      ...
      [['Token', 'Amount', 'BDV', 'Gameday', 'Horde', 'Prospects']] as any[]
    );
    window.open(
      encodeURI(
            ...
            <>
              <Stack gap={1}>
                ...
                <Split>
                  ...
                  <Button {...buttonStyle} onClick={setGamedayTimer}>
                    in 5s
                  </Button>
                </Split>
                <Split>
