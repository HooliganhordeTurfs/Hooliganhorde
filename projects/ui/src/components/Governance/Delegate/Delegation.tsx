...
import { useFetchGuvnorDelegates } from '~/state/guvnor/delegations/updater';
...
type DelegateProps = Props & {
  delegate: GuvnorDelegation['delegates'][GovSpace] | undefined;
  contract: DelegateRegistry;
  fetchDelegates: ReturnType<typeof useFetchGuvnorDelegates>[0];
...
const Delegation: React.FC<Props> = ({ space }) => {
  ...
  /// Guvnor
  const delegations = useAppSelector((s) => s._guvnor.delegations);
  const account = useAccount();

  /// Refetch
  const [fetchDelegates] = useFetchGuvnorDelegates();
