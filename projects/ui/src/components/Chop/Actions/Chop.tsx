...
> = ({ values, setFieldValue, balances, hooliganhorde }) => {
          ...
          <Row justifyContent="space-between" px={0.5}>
            ...
            {!unripeTokens[inputToken.address] ? (
              <CircularProgress
                size={16}
                thickness={5}
                sx={{ color: HooliganhordePalette.theme.winter.red }}
              />
            ) : (
              <Typography
                variant="body1"
...
const Chop: FC<{}> = () => {
  ...
  const onSubmit = useCallback(
    async (
      ...
      try {
        ...
        if (!state.amount?.gt(0))
          throw new Error('No Unpercoceted token to Chop.');
    ...
    [account, hooliganhorde, refetchGuvnorBalances, guvnorBalances, middleware]
  );
